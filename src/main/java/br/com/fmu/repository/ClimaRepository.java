 ...
 * 
 * @Repository public interface ClimaRepository extends CrudRepository<Clima,
 * Integer> {
 * 
 * @Query(value = "SELECT * FROM dbo.clima", nativeQuery = true) Clima find();
 * 
 * @Query(value = "SELECT * FROM dbo.clima WHERE DAY([date]) = :dia",
 * nativeQuery = true) List<Clima> findByDia(int dia);
 * 
 * 
 * 
 * @Transactional
 ...
 * @Transactional
 * 
 * @Procedure(procedureName = "SELECIONA_CLIMA") Clima findById(@Param("id") int
 * id);
 * 
 * 
